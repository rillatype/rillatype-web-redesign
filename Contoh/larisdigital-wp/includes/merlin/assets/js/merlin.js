var Merlin = (function ($) {
	...
	var callbacks = {
		...
		select_integration: function (btn) {
			var integration = new SelectIntegration();
			integration.init(btn);
		},
	...
	function ActivateLicense() {
		...
		var complete = $('#license-text');
		var notice = $('#license-error');
		function ajax_callback(r) {
	...
	function SelectIntegration() {
		...
		function do_ajax() {
			jQuery.post(merlin_params.ajaxurl, {
				action: "merlin_select_integration",
				wpnonce: merlin_params.wpnonce,
				integration: $('.js-integration:checked').val()
			}, ajax_callback).fail(ajax_callback);
		}
		return {
			init: function (btn) {
			...
			if (typeof response === "object" && typeof response.message !== "undefined") {
				...
				if (typeof response.done != "undefined" && response.done) {
					...
					if (response.hash == current_item_hash) {
						...
						setTimeout(function () {
							var r = confirm(merlin_params.texts.something_went_wrong);
							if (r) {
	...
	function ContentManager() {
		...
		function ajax_callback(response) {
			...
			if (typeof response == "object" && typeof response.message !== "undefined") {
				...
				if (typeof response.num_of_imported_posts !== "undefined" && 0 < total_content_import_items) {
					current_content_import_items = 'all' === response.num_of_imported_posts
		...
		function process_current() {
			if (current_item) {
				...
				if ($check.is(":checked")) {
					...
					// setTimeout(find_next,1);
				} else {
					console.log('skip ' + current_item);
					$current_node.addClass("skipping");
					setTimeout(find_next, 1);
				}
			}
		...
		function find_next() {
			...
			if ($enabled_items.length > 0) {
				...
				if (items_completed >= $enabled_items.length) {
					complete();
				}
			} else {
				complete();
		...
		function init_content_import_progress_bar() {
			if ($(".merlin__drawer--import-content__list-item .checkbox-content").is(':checked')) {
				...
				update_progress_bar_content();
				progress_bar_interval_content = setInterval(function () {
					update_progress_bar_content();
				}, 1000);
			}
		}
		...
		function update_progress_bar_content() {
			var items_total = $(".merlin__drawer--import-content__list-item input:checked").length;
			if (items_total > 0) {
				$('.js-merlin-progress-bar').css('width', (items_completed / items_total) * 100 + '%');
				var $percentage = valBetween(((items_completed / items_total) * 100), 0, 100);
				// console.log( items_completed + " / " + items_total + " = " + $percentage );
				$('.js-merlin-progress-bar-percentage').html(Math.round($percentage) + '%');
				if (1 === items_completed / items_total) {
					clearInterval(progress_bar_interval_content);
				}
			} else {
	...
	function merlin_loading_button(btn) {
		...
		var completed = false;
		var _modifier = $button.is("input") || $button.is("button")
			? "val"
			: "text";
