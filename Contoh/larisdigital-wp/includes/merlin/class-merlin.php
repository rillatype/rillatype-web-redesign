...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		add_action( 'wp_ajax_merlin_select_integration', array( $this, 'select_integration' ), 10, 0 );
	...
	public function admin_page() {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'larisdigital-wp' ),
			'timeout_alert' => esc_html__( 'There were some server errors during the import. There might be missing items.', 'larisdigital-wp' ),			
		);
		...
		if ( isset( $this->steps[ $this->step ]['name'] ) ) {
			$current_step_name = strtolower( $this->steps[ $this->step ]['name'] );
		}
		else {
			$current_step_name = ucfirst( str_replace( '_', ' ', $this->step ) );
		}
		...
		<div class="merlin__wrapper">
			...
			<div class="merlin__content merlin__content--<?php echo esc_attr( strtolower( $current_step_name ) ); ?>">
	...
	protected function header() {
		...
		<head>
			...
			<?php if ( function_exists('print_admin_styles') && function_exists('print_head_scripts') ) : ?>
			...
			<?php endif; ?>
			<style>
				.merlin__body { display: block; }
				.merlin__content { margin-top: 50px; }
				#merlin__drawer-trigger { display: none; }
				.merlin__drawer { padding-top: 20px; }
				.merlin__drawer--install-plugins li, .merlin__drawer--import-content li { padding: 2px 5px 2px 0; }
			</style>
		</head>
		<body class="merlin__body merlin__body--<?php echo esc_attr( $current_step_name ); ?> merlin__drawer--open">
		<?php
	}
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return __( 'Please define default parameters in the form of an array.', 'larisdigital-wp' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return __( 'Please define an SVG icon filename.', 'larisdigital-wp' );
		}
	...
	public function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'larisdigital-wp' ),
		...
		$this->steps['child'] = array(
			'name' => esc_html__( 'Child', 'larisdigital-wp' ),
			'view' => array( $this, 'child' ),
		);

		if ( $this->license_step_enabled ) {
			$this->steps['license'] = array(
				'name' => esc_html__( 'License', 'larisdigital-wp' ),
				'view' => array( $this, 'license' ),
			);
		}

		$this->steps['integration'] = array(
			'name' => esc_html__( 'Integration', 'larisdigital-wp' ),
			'view' => array( $this, 'integration' ),
		);

		// Show the plugin importer, only if TGMPA is included.
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'larisdigital-wp' ),
				'view' => array( $this, 'plugins' ),
			);
		}

		// Show the content importer, only if there's demo content added.
		$step_content = apply_filters( 'merlin_step_content_is_active', ! empty( $this->import_files ) );

		if ( $step_content ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'larisdigital-wp' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'larisdigital-wp' ),
	...
	<?php
		$this->logger->debug( __( 'The welcome step has been displayed', 'larisdigital-wp' ) );
	}
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( __( 'The license activation step has been displayed', 'larisdigital-wp' ) );
	}
	...
	<?php
		$this->logger->debug( __( 'The child theme installation step has been displayed', 'larisdigital-wp' ) );
	}
	...
	protected function integration() {
		...
		$integrations = apply_filters( 'merlin_get_integrations', array() );
		$count        = count( $integrations );

		$integration_option = apply_filters( 'merlin_get_integration_option', '' );
		$integration_active = !empty($integration_option) ? get_option( $integration_option ) : '';
		if ( empty($integration_active) && !empty($integrations) ) {
			$integration_active = current(array_keys($integrations));
		}
		...
		// Text strings.
		$header    = $strings['integration-header'];
		$paragraph = $strings['integration'];
		$action    = $strings['integration-action-link'];
		$skip      = $strings['btn-skip'];
		$next      = $strings['btn-next'];
		$select    = $strings['btn-integration-select'];
		?>
		...
		<div class="merlin__content--transition">
			...
			<p id="integration-text"><?php echo esc_html( $paragraph ); ?></p>
		...
		<form action="" method="post">
			...
			<?php if ( $count ) : ?>
				...
				<ul class="merlin__drawer merlin__drawer--integration">

				<?php if ( ! empty( $integrations ) ) : ?>
					<?php foreach ( $integrations as $integration => $data ) : ?>
						<li data-slug="<?php echo esc_attr( $integration ); ?>">
							<input type="radio" name="integration" class="radio js-integration" id="integration_<?php echo esc_attr( $integration ); ?>" value="<?php echo esc_attr( $integration ); ?>" <?php if ( !$data['active'] ) echo ' disabled '; ?> <?php if ( $integration == $integration_active ) echo ' checked '; ?>>

							<label for="integration_<?php echo esc_attr( $integration ); ?>">

								<span><?php echo esc_html( $data['name'] ); ?></span>

								<?php if ( !$data['active'] ) : ?>
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_html_e( 'Coming Soon', 'larisdigital-wp' ); ?>">
										<?php esc_html_e( 'coming soon', 'larisdigital-wp' ); ?>
									</span>
								</span>
								<?php endif; ?>
							</label>
			...
			<footer class="merlin__content__footer">
				...
				<a href="<?php echo esc_url( $this->step_next_link() ); ?>" class="merlin__button merlin__button--next button-next" data-callback="select_integration">
					<span class="merlin__button--loading__text"><?php echo esc_html( $select ); ?></span>
					<?php echo wp_kses( $this->loading_spinner(), $this->loading_spinner_allowed_html() ); ?>
				</a>
	...
	<?php
		$this->logger->debug( __( 'The integration step has been displayed', 'larisdigital-wp' ) );
	}
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_html_e( 'Required', 'larisdigital-wp' ); ?>">
										<?php esc_html_e( 'req', 'larisdigital-wp' ); ?>
									</span>
								</span>
							</label>
						</li>
	...
	<?php
		$this->logger->debug( __( 'The plugin installation step has been displayed', 'larisdigital-wp' ) );
	}
	...
	protected function content() {
		...
		if ( get_option( 'merlin_' . $this->slug . '_completed' ) ) {
			$paragraph = $strings['import_again'];
		}
		else {
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					...
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'larisdigital-wp' ); ?>">
	...
	<?php
		$this->logger->debug( __( 'The content import step has been displayed', 'larisdigital-wp' ) );
	}
	...
	protected function ready() {
		...
		for ( $i = 1; $i <= 10; $i++ ) {
			if ( isset( $strings[ "ready-link-$i" ] ) && ! empty( $strings[ "ready-link-$i" ] ) ) {
	...
	<?php
		$this->logger->debug( __( 'The final step has been displayed', 'larisdigital-wp' ) );
	}
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			$this->logger->debug( __( 'The existing child theme was activated', 'larisdigital-wp' ) );
		...
		$this->logger->debug( __( 'The newly generated child theme was activated', 'larisdigital-wp' ) );
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'larisdigital-wp' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'larisdigital-wp' ),
				)
			);
		}
		...
		$this->logger->debug( __( 'The license activation was performed with the following results', 'larisdigital-wp' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'larisdigital-wp' );
			}
		} else {
			...
			if ( false === $license_data->success ) {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Lisensi Anda sudah kadaluarsa pada tanggal %s.', 'larisdigital-wp' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Kode lisensi Anda sudah di-nonaktif-kan. Silahkan hubungi support.', 'larisdigital-wp' );
						break;

					case 'missing':
						$message = esc_html__( 'Kode lisensi Anda sepertinya salah. Silahkan coba lagi atau hubungi support.', 'larisdigital-wp' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Lisensi Anda belum aktif di website ini.', 'larisdigital-wp' );
						break;

					case 'invalid_item_id':
					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'Anda salah menggunakan kode lisensi untuk produk lain. Kode lisensi ini bukan untuk %s. Silahkan ganti dengan kode lisensi yang benar.', 'larisdigital-wp' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Maaf, kode lisensi Anda sudah mencapai batas limit maksimal aktivasi website. Silahkan deactivate lisensi di web yang lain supaya Anda bisa menggunakannya kembali di website ini.', 'larisdigital-wp' );
						break;

					default:
						$message = esc_html__( 'ERROR! Silahkan coba lagi atau hubungi support', 'larisdigital-wp' );
						break;
				}
			} else {
				if ( 'valid' === $license_data->license ) {
					...
					do_action( 'merlin_before_edd_activate_license' );

					// Removes the default EDD hook for this option, which breaks the AJAX call.
					remove_all_actions( 'update_option_' . $this->edd_theme_slug . '_license_key' );
					remove_all_actions( 'add_option_' . $this->edd_theme_slug . '_license_key' );
					...
					do_action( 'merlin_after_edd_activate_license' );
				}
			}
		}
	...
	public function generate_child_functions_php( $slug ) {
		...
		$this->logger->debug( __( 'The child theme functions.php content was generated', 'larisdigital-wp' ) );
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$this->logger->debug( __( 'The child theme style.css content was generated', 'larisdigital-wp' ) );
	...
	public function generate_child_screenshot( $path ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( __( 'The child theme screenshot was copied to the child theme, with the following result', 'larisdigital-wp' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( __( 'The child theme screenshot was not generated, because of these results', 'larisdigital-wp' ), array( 'screenshot' => $screenshot ) );
		}
	}

	/**
	 * Select Integration via AJAX.
	 */
	public function select_integration() {
		...
		if ( !empty( $_POST['integration'] ) ) {
			...
			if ( !empty($integration_option) ) {
				...
				update_option( $integration_option, $integration );
			}
		}
		...
		wp_send_json(
			array(
				'done' => 1,
				'message' => $strings['integration-success'],
			)
		);
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => -1,
					'message'       => esc_html__( 'Activating', 'larisdigital-wp' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => -1,
					'message'       => esc_html__( 'Updating', 'larisdigital-wp' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => -1,
					'message'       => esc_html__( 'Installing', 'larisdigital-wp' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				__( 'A plugin with the following data will be processed', 'larisdigital-wp' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'larisdigital-wp' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				__( 'A plugin with the following data was processed', 'larisdigital-wp' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'larisdigital-wp' ),
				)
			);
		}
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			$this->logger->error( __( 'The content importer AJAX call failed to start, because of incorrect data', 'larisdigital-wp' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'larisdigital-wp' ),
				)
			);
		}
		...
		if ( isset( $content[ $_POST['content'] ] ) ) {
			$this_content = $content[ $_POST['content'] ];
		}
		else {
			$this->logger->error(
				__( 'The content import AJAX call failed because the data is not available', 'larisdigital-wp' ),
				array(
			...
			wp_send_json(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'larisdigital-wp' ),
					'logs'    => '',
					'errors'  => '',
				)
			);
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					__( 'The content import AJAX call will be executed with this import data', 'larisdigital-wp' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				__( 'The content import AJAX call failed with this passed data', 'larisdigital-wp' ),
				array(
					'selected_content_index' => $selected_import,
					'importing_content'      => $_POST['content'],
	...
	public function _ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			$this->logger->error( __( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'larisdigital-wp' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'larisdigital-wp' ),
				)
			);
		}
	...
	public function get_import_data_info( $selected_import_index = 0 ) {
		...
		$content = $this->get_import_data( $selected_import_index );
		foreach ( $content as $key => $value ) {
			if ( ! isset( $import_data[$key] ) ) {
				$import_data[$key] = isset( $value['title'] ) && $value['title'] ? $value['title'] : true;
			}
		}
		...
		if (
			...
			$import_data['content'] = isset( $content['content']['title'] ) && $content['content']['title'] ? $content['content']['title'] : true;
		}
		...
		if (
			...
			$import_data['widgets'] = isset( $content['widgets']['title'] ) && $content['widgets']['title'] ? $content['widgets']['title'] : true;
		}
		...
		if (
			...
			$import_data['options'] = isset( $content['options']['title'] ) && $content['options']['title'] ? $content['options']['title'] : true;
		}
		...
		if (
			...
			$import_data['sliders'] = isset( $content['sliders']['title'] ) && $content['sliders']['title'] ? $content['sliders']['title'] : true;
		}
		...
		if (
			...
			$import_data['redux'] = isset( $content['redux']['title'] ) && $content['redux']['title'] ? $content['redux']['title'] : true;
		}

		if ( $import_data['content'] || $import_data['widgets'] || $import_data['options'] || $import_data['sliders'] || $import_data['redux'] ) {
			if ( false !== has_action( 'merlin_after_all_import' ) ) {
				$import_data['after_import'] = isset( $content['after_import']['title'] ) && $content['after_import']['title'] ? $content['after_import']['title'] : true;
			}
		}
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( class_exists( 'ProteusThemes\WPContentImporter2\Importer' ) ) {
			if ( ! empty( $import_files['content'] ) ) {
				$content['content'] = array(
					'title'            => esc_html__( 'Content', 'larisdigital-wp' ),
					'description'      => esc_html__( 'Demo content data.', 'larisdigital-wp' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'larisdigital-wp' ),
				'description'      => esc_html__( 'Sample widgets data.', 'larisdigital-wp' ),
				'pending'          => esc_html__( 'Pending', 'larisdigital-wp' ),
				'installing'       => esc_html__( 'Installing', 'larisdigital-wp' ),
				'success'          => esc_html__( 'Success', 'larisdigital-wp' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'larisdigital-wp' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'larisdigital-wp' ),
				'pending'          => esc_html__( 'Pending', 'larisdigital-wp' ),
				'installing'       => esc_html__( 'Installing', 'larisdigital-wp' ),
				'success'          => esc_html__( 'Success', 'larisdigital-wp' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'larisdigital-wp' ),
				'description'      => esc_html__( 'Sample theme options data.', 'larisdigital-wp' ),
				'pending'          => esc_html__( 'Pending', 'larisdigital-wp' ),
				'installing'       => esc_html__( 'Installing', 'larisdigital-wp' ),
				'success'          => esc_html__( 'Success', 'larisdigital-wp' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'larisdigital-wp' ),
				'description'      => esc_html__( 'Redux framework options.', 'larisdigital-wp' ),
				'pending'          => esc_html__( 'Pending', 'larisdigital-wp' ),
				'installing'       => esc_html__( 'Installing', 'larisdigital-wp' ),
				'success'          => esc_html__( 'Success', 'larisdigital-wp' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'larisdigital-wp' ),
				'description'      => esc_html__( 'After import setup.', 'larisdigital-wp' ),
				'pending'          => esc_html__( 'Pending', 'larisdigital-wp' ),
				'installing'       => esc_html__( 'Installing', 'larisdigital-wp' ),
				'success'          => esc_html__( 'Success', 'larisdigital-wp' ),
		...
		$content = apply_filters( 'merlin_get_base_content', $content, $selected_import_index, $this );
	...
	public function import_revolution_sliders( $file ) {
		...
		$this->logger->info( __( 'The revolution slider import was executed', 'larisdigital-wp' ) );
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'larisdigital-wp' );
		$data['proceed']  = 'true';
	...
	public function after_content_import_setup() {
		...
		if ( $homepage ) {
			...
			$this->logger->debug( __( 'The home page was set', 'larisdigital-wp' ), array( 'homepage_id' => $homepage ) );
		}
		...
		if ( $blogpage ) {
			...
			$this->logger->debug( __( 'The blog page was set', 'larisdigital-wp' ), array( 'blog_page_id' => $blogpage ) );
		}
	}
	...
	public function before_content_import_setup() {
		...
		if ( ! empty( $hello_world ) ) {
			...
			$this->logger->debug( __( 'The Hello world post status was set to draft', 'larisdigital-wp' ) );
		}
	}
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( __( 'This predefined demo import does not have the name parameter: import_file_name', 'larisdigital-wp' ), $import_file );
			}
		}
	...
	public function get_import_steps_html( $import_info ) {
		...
		$checked = get_option( 'merlin_' . $this->slug . '_completed' ) ? '' : 'checked';
		...
		?>
			<?php foreach ( $import_info as $slug => $available ) : ?>
				...
				if ( ! empty( $available ) && $available !== true ) {
					$label = $available;
				}
				else {
				...
				<li class="merlin__drawer--import-content__list-item status status--Pending" data-content="<?php echo esc_attr( $slug ); ?>">
					<input type="checkbox" name="default_content[<?php echo esc_attr( $slug ); ?>]" class="checkbox checkbox-<?php echo esc_attr( $slug ); ?>" id="default_content_<?php echo esc_attr( $slug ); ?>" value="1" <?php echo esc_html( $checked ); ?>>
					<label for="default_content_<?php echo esc_attr( $slug ); ?>">
						<i></i><span><?php echo esc_html( $label ); ?></span>
					</label>
				</li>
