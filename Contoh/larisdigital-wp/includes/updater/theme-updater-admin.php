...
class LarisDigital_Theme_Updater_Admin {
	...
	function updater() {
		...
		if ( !class_exists( 'LarisDigital_Theme_Updater' ) ) {
			// Load our custom theme updater
			include( dirname( __FILE__ ) . '/theme-updater-class.php' );
		}

		new LarisDigital_Theme_Updater(
			array(
	...
	function license_page() {
		...
		?>
		<style>
		.larisdigital-license-form {
			padding: 10px 20px;
			background: #fff;
			border-left: 4px solid #00a0d2;
			margin-top: 15px;
		}
		.larisdigital-license-form input {
			height: 40px;
			line-height: 40px;
			padding: 0 10px;
			vertical-align: top;
		...
		.wp-core-ui .larisdigital-license-form .button, .wp-core-ui .larisdigital-license-form .button-primary, .wp-core-ui .larisdigital-license-form .button-secondary {
			height: 40px;
			line-height: 40px;
			padding: 0 20px;
			vertical-align: top;
		}
		.larisdigital-license-form a {
			text-decoration: none;
		}
		.larisdigital-license-good {
			color: #3c763d;
		}
		.larisdigital-license-bad {
			color: #a94442;
		}
		</style>
		<div class="wrap">
			<h2><?php echo $strings['theme-license'] ?></h2>
			<form method="post" action="options.php" class="larisdigital-license-form">

				<?php settings_fields( $this->theme_slug . '-license' ); ?>
				<?php wp_nonce_field( $this->theme_slug . '_nonce', $this->theme_slug . '_nonce' ); ?>

				<p>Kode lisensi LarisDigital WooCommerce WordPress Theme dari TokoPressID yang aktif dibutuhkan untuk mendapatkan update otomatis dan support teknis.</p>
				...
				<?php if ( $license ) : ?>
					<?php if ( in_array( $status, array( 'valid' ) ) ) : ?>
						...
						<p><span class="description">
							<span class="larisdigital-license-good"><b>STATUS:</b> <?php echo $message; ?></span>
						</span></p>
					<?php elseif ( in_array( $status, array( 'site_inactive' ) ) ) : ?>
						...
						<p><span class="description">
							<span class="larisdigital-license-bad"><b>STATUS:</b> <?php echo $message; ?></span>
						</span></p>
					<?php else : ?>
						<p>
				...
				<?php else : ?>
					...
					<p><span class="description">
						<span class="larisdigital-license-bad"><?php echo $message; ?></span>
					</span></p>
				<?php endif; ?>
				...
				<p>
					<ol>
						<li>
							<b><a href="https://www.tokopress.id/my-account" target="_blank">Login ke member area</a></b>, jika Anda SUDAH pernah membeli LarisDigital WooCommerce WordPress Theme ini.
						</li>
						<li>
							<b><a href="https://www.tokopress.id/" target="_blank">Beli LarisDigital theme</a></b>, Jika Anda BELUM pernah membeli LarisDigital WooCommerce WordPress Theme ini.
						</li>
					</ol>
				</p>
			</form>
	 ...
	 */
	 function get_api_response( $api_params ) {
		...
		// 	wp_die( $response->get_error_message(), __( 'Error', 'larisdigital-wp' ) . $response->get_error_code() );
		// }
	...
	function activate_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = __( 'An error occurred, please try again.', 'larisdigital-wp' );
			}
			...
			if ( false === $license_data->success ) {
				...
				switch( $license_data->error ) {
					...
					case 'expired' :
						...
						$message = sprintf(
							__( 'Kode lisensi Anda telah kadaluarsa pada %s.', 'larisdigital-wp' ),
					...
					case 'revoked' :
						...
						$message = __( 'Kode lisensi Anda telah dinonaktifkan dan tidak dapat dipergunakan lagi.', 'larisdigital-wp' );
						break;

					case 'missing' :

						$message = __( 'Lisensi tidak valid.', 'larisdigital-wp' );
						break;

					case 'invalid' :
					case 'site_inactive' :

						$message = __( 'Lisensi Anda sedang tidak aktif di website ini.', 'larisdigital-wp' );
						break;

					case 'item_name_mismatch' :

						$message = sprintf( __( 'Kode lisensi ini sepertinya tidak valid untuk %s.', 'larisdigital-wp' ), $this->item_name );
						break;

					case 'no_activations_left':

						$message = __( 'Kode lisensi Anda telah mencapai batas limit aktivasi lisensi.', 'larisdigital-wp' );
						break;
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				if ( strpos( $message, 'Could not resolve host' ) !== false ) {
					$message = esc_html__( 'Could not connect to TokoPressID license server', 'larisdigital-wp' );
				}
			} 
			else {
...
function larisdigital_theme_license_admin_notices() {
...
add_action( 'admin_notices', 'larisdigital_theme_license_admin_notices', 999 );
